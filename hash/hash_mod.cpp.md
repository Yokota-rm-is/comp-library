...
data:
  ...
  bundledCode: "#line 2 \"base.cpp\"\n\n#include <bits/stdc++.h>\n// #include <atcoder/all>\n\
    ...
    \ hash;\n    size_t size;\n\n    HashMod(long long N) : hash(mods.size()), size(mods.size())\
    \ {construct(N);}\n    HashMod(string S) : hash(mods.size()), size(mods.size())\
    \ { construct(S);}\n    HashMod(mpz_class S) : hash(mods.size()), size(mods.size())\
    \ { construct(S);}\n\n    void construct(long long N) {\n        rep(i, size)\
    \ hash[i] = N % mods[i];\n    }\n\n    void construct(string S) { construct(mpz_class(S));}\n\
    \    void construct(mpz_class S) {\n        rep(i, size) {\n            mpz_class\
    \ x = S % mpz_class(lltos(mods[i]));\n            hash[i] = (long long)x.get_ui();\n\
    \        }\n    }\n    \n    HashMod operator + (const HashMod& r) const noexcept\
    \ { return HashMod(*this) += r; }\n    HashMod operator - (const HashMod& r) const\
    \ noexcept { return HashMod(*this) -= r; }\n    HashMod operator * (const HashMod&\
    \ r) const noexcept { return HashMod(*this) *= r; }\n    HashMod operator / (const\
    \ HashMod& r) const noexcept { return HashMod(*this) /= r; }\n    HashMod& operator\
    \ += (const HashMod& r) noexcept {\n        rep(i, size) {\n            hash[i]\
    \ += r.hash[i];\n            if (hash[i] >= mods[i]) hash[i] -= mods[i];\n   \
    \     }\n        return *this;\n    }\n    constexpr HashMod& operator -= (const\
    \ HashMod& r) noexcept {\n        rep(i, size) {\n            hash[i] -= r.hash[i];\n\
    \            if (hash[i] < 0) hash[i] += mods[i];\n        }\n        return *this;\n\
    \    }\n    constexpr HashMod& operator *= (const HashMod& r) noexcept {\n   \
    \     rep(i, size) {\n            hash[i] = hash[i] * r.hash[i] % mods[i];\n \
    \       }\n        return *this;\n    }\n    constexpr HashMod& operator /= (const\
    \ HashMod& r) noexcept {\n        rep(i, size) {\n            hash[i] = hash[i]\
    \ * r.hash[i] % mods[i];\n            long long a = r.hash[i], b = mods[i], u\
    \ = 1, v = 0;\n\n            while (b) {\n                long long t = a / b;\n\
    \                a -= t * b; swap(a, b);\n                u -= t * v; swap(u,\
    \ v);\n            }\n\n            hash[i] = hash[i] * u % mods[i];\n       \
    \     if (hash[i] < 0) hash[i] += mods[i];\n        }\n\n        return *this;\n\
    \    }\n    constexpr bool operator == (const HashMod& r) const noexcept {\n \
    \       bool ret = true;\n        rep(i, size) {\n            ret &= (hash[i]\
    \ == r.hash[i]);\n        }\n        return ret;\n    }\n    constexpr bool operator\
    \ != (const HashMod& r) const noexcept {\n        bool ret = false;\n        rep(i,\
    \ size) {\n            ret |= (hash[i] != r.hash[i]);\n        }\n        return\
    \ ret;\n    }\n\n    constexpr bool operator< (const HashMod &r) const noexcept\
  ...
  verifiedWith: []
documentation_of: hash/hash_mod.cpp
layout: document
redirect_from:
- /library/hash/hash_mod.cpp
- /library/hash/hash_mod.cpp.html
