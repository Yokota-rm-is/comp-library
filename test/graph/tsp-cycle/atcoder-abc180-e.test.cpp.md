...
data:
  ...
  bundledCode: "#line 1 \"test/graph/tsp-cycle/atcoder-abc180-e.test.cpp\"\n#define\
    \ PROBLEM \"https://atcoder.jp/contests/abc180/tasks/abc180_e\"\n\n#line 2 \"\
    ...
    \ = bit | (1 << next);\n                    if (chmin(dp[next_bit][next], dp[bit][now]\
    ...
    \     if (bit < 0) bit = (1 << V) - 1;\n        bit &= ~(1 << _start);\n\n   \
    \     return min(dp[bit]);\n    }\n\n    vector<long long> get_path(long long\
    \ goal, long long bit = -1) {\n        if (bit < 0) bit = (1 << V) - 1;\n\n  \
    \      vector<long long> p;\n        p.push_back(goal);\n\n        long long to\
    \ = goal;\n        while (true) {\n            long long from = prev[bit][to];\n\
    \            if (from == -1) break;\n\n            p.push_back(from);\n      \
    \      bit &= ~(1 << to);\n            to = from;\n        }\n\n        reverse(p.begin(),\
    \ p.end());\n\n        return p;\n    }\n\n    vector<long long> get_min_path(long\
    \ long bit = -1) {\n        if (bit < 0) bit = (1 << V) - 1;\n\n        long long\
    \ goal = min_element(dp[bit].begin(), dp[bit].end()) - dp[bit].begin();\n    \
    \    return get_path(goal, bit);\n    }\n};\n#line 4 \"test/graph/tsp-cycle/atcoder-abc180-e.test.cpp\"\
    \n\nint main() {\n    ll N;\n    cin >> N;\n\n    vll X(N), Y(N), Z(N);\n    rep(i,\
    \ N) cin >> X[i] >> Y[i] >> Z[i];\n\n    TSP tsp(N, true);\n    rep(i, N) rep(j,\
  ...
  requiredBy: []
  timestamp: '2025-03-23 19:04:04+09:00'
  verificationStatus: TEST_WRONG_ANSWER
  verifiedWith: []
documentation_of: test/graph/tsp-cycle/atcoder-abc180-e.test.cpp
layout: document
redirect_from:
- /verify/test/graph/tsp-cycle/atcoder-abc180-e.test.cpp
- /verify/test/graph/tsp-cycle/atcoder-abc180-e.test.cpp.html
