...
data:
  ...
  bundledCode: "#line 1 \"test/graph/psp/yukicoder-1541.test.cpp\"\n#define PROBLEM\
    \ \"https://yukicoder.me/problems/no/1541\"\n\n#line 2 \"base.cpp\"\n\n#include\
    ...
    struct PSP {\n    struct Edge {\n        long long from;\n        long long to;\n\
    \        Cap cap, flow;\n        long long idx;\n        long long rev;\n    \
    \    \n        Edge(long long u = -1, long long v = -1, Cap cap = 0, long long\
    \ i = 0, long long rev = 0) : from(u), to(v), cap(cap), flow(0), idx(i), rev(rev)\
    ...
    \ edge_index = 0;\n    vector<Edge> edges;\n\n    // maxflow\u7528\n    vector<long\
    \ long> level;\n\n    PSP(long long N) : V(N + 2), G(V), start(N), goal(N + 1),\
    \ offset(0) {\n        init();\n    };\n    \n    void init() {}\n\n    void connect(long\
    \ long from, long long to, Cap cap) {\n        assert(0 <= from and from < V);\n\
    ...
    \        fore(b, B[i]) cin >> b;\n    }\n\n    // true: \u52C9\u5F37\u3059\u308B\
    \n    // false: \u52C9\u5F37\u3057\u306A\u3044\n    PSP<ll> flow(N);\n\n    rep(i,\
    \ N) {\n        flow.set_gain_if_true(i, M);\n        flow.set_cost_if_true(i,\
    \ C[i]);\n\n        rep(j, k[i]) {\n            flow.set_gain_if_both_true(i,\
    \ A[i][j], B[i][j]);\n        }\n    }\n\n    ll ans = -flow.solve();\n    cout\
    \ << ans << endl;\n\n    return 0;\n}\n"
  ...
  verifiedWith: []
documentation_of: test/graph/psp/yukicoder-1541.test.cpp
layout: document
redirect_from:
- /verify/test/graph/psp/yukicoder-1541.test.cpp
- /verify/test/graph/psp/yukicoder-1541.test.cpp.html
