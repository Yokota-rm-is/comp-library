...
data:
  ...
  bundledCode: "#line 1 \"test/graph/flow/atcoder-abc247-g.test.cpp\"\n#define IGNORE\n\
    ...
    \ * cost));\n        }\n\n        cur_flow += flow;\n        cur_cost += flow\
    \ * cost;\n        pre_cost = cost;\n    }\n};\n#line 6 \"test/graph/flow/atcoder-abc247-g.test.cpp\"\
    \n\nint main() {\n    ll N;\n    cin >> N;\n\n    vll A(N), B(N), C(N);\n    rep(i,\
    \ N) cin >> A[i] >> B[i] >> C[i];\n    --A, --B;\n\n    ll H = 150, W = 150;\n\
    \    ll inf = 1e9;\n    vvll grid(H, vll(W, inf));\n    rep(i, N) {\n        grid[A[i]][B[i]]\
    \ = min(grid[A[i]][B[i]], inf - C[i]);\n    }\n\n    MinCostFlow<ll, ll> mcf(H\
    \ + W + 2);\n    ll s = H + W, t = s + 1;\n    rep(i, H) {\n        mcf.connect(s,\
    \ i, 1, 0);\n    }\n    rep(i, H) rep(j, W) {\n        if (grid[i][j] == inf)\
    \ continue;\n        mcf.connect(i, H + j, 1, grid[i][j]);\n    }\n    rep(j,\
    \ W) {\n        mcf.connect(H + j, t, 1, 0);\n    }\n\n    auto slope = mcf.slope(s,\
  ...
  verifiedWith: []
documentation_of: test/graph/flow/atcoder-abc247-g.test.cpp
layout: document
redirect_from:
- /verify/test/graph/flow/atcoder-abc247-g.test.cpp
- /verify/test/graph/flow/atcoder-abc247-g.test.cpp.html
