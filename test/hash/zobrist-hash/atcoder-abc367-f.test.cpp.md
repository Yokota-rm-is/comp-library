...
data:
  ...
  bundledCode: "#line 1 \"test/hash/zobrist-hash/atcoder-abc367-f.test.cpp\"\n#define\
    \ PROBLEM \"https://atcoder.jp/contests/abc367/tasks/abc367_f\"\n\n#line 2 \"\
    ...
    \        else hash ^= rng(x); \n    }\n    void erase(ull x) { \n        if constexpr\
    \ (multi) hash -= rng(x);\n        else hash ^= rng(x);\n    }\n\n    ZobristHash\
    \ operator+(const ZobristHash& rhs) const { \n        if constexpr (multi) return\
    \ ZobristHash<multi>(hash + rhs.hash); \n        else return ZobristHash<multi>(hash\
    ...
    \ operator+(ull h) const { \n        if constexpr (multi) return ZobristHash<multi>(hash\
    \ + rng(h)); \n        else return ZobristHash<multi>(hash ^ rng(h));\n    }\n\
    \    ZobristHash operator-(ull h) const { \n        if constexpr (multi) return\
    \ ZobristHash<multi>(hash - rng(h)); \n        else return ZobristHash<multi>(hash\
    \ ^ rng(h));\n    }\n    \n    bool operator==(const ZobristHash& rhs) const {\
    \ return hash == rhs.hash; }\n    bool operator!=(const ZobristHash& rhs) const\
    \ { return hash != rhs.hash; }\n};\n#line 4 \"test/hash/zobrist-hash/atcoder-abc367-f.test.cpp\"\
    \n\nint main() {\n    ll N, Q;\n    cin >> N >> Q;\n\n    vll A(N), B(N);\n  \
    \  rep(i, N) cin >> A[i];\n    rep(i, N) cin >> B[i];\n\n    vector<ZobristHash<true>>\
    \ hashA(N + 1), hashB(N + 1);\n    rep(i, N) {\n        hashA[i + 1] = hashA[i];\n\
    \        hashA[i + 1].insert(A[i]);\n\n        hashB[i + 1] = hashB[i];\n    \
    \    hashB[i + 1].insert(B[i]);\n    }\n    \n    rep(i, Q) {\n        ll l, r,\
    \ L, R;\n        cin >> l >> r >> L >> R;\n\n        ZobristHash hash_lr = hashA[r]\
  ...
  verifiedWith: []
documentation_of: test/hash/zobrist-hash/atcoder-abc367-f.test.cpp
layout: document
redirect_from:
- /verify/test/hash/zobrist-hash/atcoder-abc367-f.test.cpp
- /verify/test/hash/zobrist-hash/atcoder-abc367-f.test.cpp.html
