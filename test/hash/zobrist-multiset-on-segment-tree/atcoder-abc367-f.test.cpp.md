...
data:
  ...
  bundledCode: "#line 1 \"test/hash/zobrist-multiset-on-segment-tree/atcoder-abc367-f.test.cpp\"\
    ...
    \    return result;\n}\n#line 3 \"hash/zobrist-multiset-on-segment-tree.cpp\"\n\
    \n// \u6574\u6570\u5024 x \u306B\u30CF\u30C3\u30B7\u30E5\u5024\u3092\u5272\u308A\
    \u5F53\u3066\u308B\u95A2\u6570\nstruct CustomHash {\n    using ull = unsigned\
    ...
    \n    ZobristMultisetonSegmentTree() {}\n    ZobristMultisetonSegmentTree(long\
    \ long n) { \n        init(n);\n    }\n    ZobristMultisetonSegmentTree(vector<long\
    \ long> A) { \n        init(A);\n    }\n    ZobristMultisetonSegmentTree(vector<unsigned\
    \ long long> A) { \n        init(A);\n    }\n\n    void init(long long n) {\n\
    \        _N = n;\n        N = 1;\n        height = 1;\n        while (N < (_N\
    \ + 1)) {\n            N *= 2;\n            ++height;\n        }\n        \n \
    \       node.assign(2 * N, 0);\n        values.assign(N, 0);\n    }\n\n    void\
    \ init(vector<long long> A) {\n        init(A.size());\n\n        rep(i, _N) {\n\
    \            node[i + N] = rng(A[i]);\n            values[i] = A[i];\n       \
    \ }\n        repd(i, 1, N) update(i);\n    }\n\n    void init(vector<unsigned\
    \ long long> A) {\n        init(A.size());\n\n        rep(i, _N) {\n         \
    \   node[i + N] = rng(A[i]);\n            values[i] = A[i];\n        }\n     \
    \   repd(i, 1, N) update(i);\n    }\n\n    // \u4F4D\u7F6Ep (0-indexed)\u306E\u5024\
    \u3092x\u306B\u3059\u308B\n    void set(long long p, unsigned long long x) {\n\
    \        assert(0 <= p and p <= _N);\n\n        long long k = p + N;\n       \
    \ node[k] = rng(x);\n        values[p] = x;\n        rep(i, 1, height) update(k\
    \ >> i);\n    }\n\n    void add(long long p, unsigned long long x) {\n       \
    \ assert(0 <= p and p <= _N);\n\n        long long k = p + N;\n        values[p]\
    \ += x;\n        node[k] = rng(values[p]);\n        rep(i, 1, height) update(k\
    \ >> i);\n    }\n\n    // \u534A\u958B\u533A\u9593[l, r) (0-indexed)\u306E\u7DCF\
    \u548C\u3092\u6C42\u3081\u308B\n    unsigned long long prod(long long l, long\
    \ long r) {\n        assert(0 <= l && l <= r && r <= _N);\n\n        l += N;\n\
    ...
    \      node[k] = node[k * 2] + node[k * 2 + 1];\n    }\n};\n#line 6 \"test/hash/zobrist-multiset-on-segment-tree/atcoder-abc367-f.test.cpp\"\
    \n\nint main() {\n    ll N, Q;\n    cin >> N >> Q;\n\n    vll A(N), B(N);\n  \
    \  rep(i, N) cin >> A[i];\n    rep(i, N) cin >> B[i];\n\n    ZobristMultisetonSegmentTree\
    \ treeA(A), treeB(B);\n\n    while (Q--) {\n        ll l, r, L, R;\n        cin\
    \ >> l >> r >> L >> R;\n        --l, --L;\n\n        cout << YesNo(treeA.prod(l,\
    \ r) == treeB.prod(L, R)) << endl;\n    }\n\n    return 0;\n} \n"
  code: "#define IGNORE\n#define PROBLEM \"https://atcoder.jp/contests/abc367/tasks/abc367_f\"\
  ...
  verifiedWith: []
documentation_of: test/hash/zobrist-multiset-on-segment-tree/atcoder-abc367-f.test.cpp
layout: document
redirect_from:
- /verify/test/hash/zobrist-multiset-on-segment-tree/atcoder-abc367-f.test.cpp
- /verify/test/hash/zobrist-multiset-on-segment-tree/atcoder-abc367-f.test.cpp.html
