...
data:
  ...
  bundledCode: "#line 1 \"test/structure/dynamic-binary-indexed-tree-2d/yosupo-point-add-rectangle-sum.test.cpp\"\
    ...
    \ add(long long h, long long w, T x) {\n        assert(0 <= h and h < _H and 0\
    \ <= w and w < _W);\n\n        ++h; ++w; // 1-indexed\n\n        for (long long\
    \ i = h; i < H; i += (i & -i)) {\n            for (long long j = w; j < W; j +=\
    \ (j & -j)) {\n                bit[i][j] += x;\n            }\n        }\n   \
    \ }\n\n    // \u4F4D\u7F6Eh, w (0-indexed)\u3092x\u306B\u3059\u308B\n    void\
    \ set(long long h, long long w, T x) {\n        assert(0 <= h and h < _H and 0\
    \ <= w and w < _W);\n\n        add(h, w, x - get(h, w));\n    }\n\n    // \u534A\
    \u958B\u533A\u9593[0, h) x [0, w) (0-indexed)\u306E\u7DCF\u548C\u3092\u6C42\u3081\
    ...
    \ h + 1, w + 1);\n    }\n\n    friend ostream& operator << (ostream& os, DynamicBinaryIndexedTree2D&\
    \ bit) {\n        os << \"bit\" << endl;\n        rep(h, bit.H) {\n          \
    \  rep(w, bit.W) {\n                os << bit.bit[h][w] << \" \";\n          \
    \  }\n            os << endl;\n        }\n        os << endl;\n\n        os <<\
    \ \"value\" << endl;\n        rep(i, bit._H) {\n            rep(j, bit._W) {\n\
    ...
    #line 5 \"test/structure/dynamic-binary-indexed-tree-2d/yosupo-point-add-rectangle-sum.test.cpp\"\
    \n\nint main() {\n    ll N, Q;\n    cin >> N >> Q;\n\n    vector<ll> X, Y, W;\n\
    \n    rep(i, N) {\n        ll x, y, w;\n        cin >> x >> y >> w;\n\n      \
    \  X.push_back(x);\n        Y.push_back(y);\n        W.push_back(w);\n    }\n\n\
    \    vector<vll> query;\n\n    rep(q, Q) {\n        ll t;\n        cin >> t;\n\
  ...
  code: "#define PROBLEM \"https://judge.yosupo.jp/problem/point_add_rectangle_sum\"\
    ...
    \ bit(comp_x.size() + 1, comp_y.size() + 1);\n\n    rep(i, N) {\n        ll x\
    \ = comp_x.find(X[i]);\n        ll y = comp_y.find(Y[i]);\n        ll w = W[i];\n\
    \n        bit.add(x, y, w);\n    }\n\n    rep(q, Q) {\n        ll t = query[q][0];\n\
    \n        if (t == 0) {\n            ll x = comp_x.find(query[q][1]);\n      \
    \      ll y = comp_y.find(query[q][2]);\n            ll w = query[q][3];\n\n \
  ...
  requiredBy: []
  timestamp: '2024-09-28 16:33:22+09:00'
  verificationStatus: TEST_ACCEPTED
  verifiedWith: []
documentation_of: test/structure/dynamic-binary-indexed-tree-2d/yosupo-point-add-rectangle-sum.test.cpp
layout: document
redirect_from:
- /verify/test/structure/dynamic-binary-indexed-tree-2d/yosupo-point-add-rectangle-sum.test.cpp
- /verify/test/structure/dynamic-binary-indexed-tree-2d/yosupo-point-add-rectangle-sum.test.cpp.html
