...
// root_ceil関数の検証関数
bool validate_ceil_floor(long long x, long long n, long long ret) {
    __int128_t x_ = x;

    // ret^n を計算
    __int128_t prev_power_n = binpow<__uint128_t>(ret - 1, n);
    ...
    cout << "Expected: " << to_string(prev_power_n) << " < " << x << " <= " << to_string(ret_power_n) << endl;
...
int main() {
    ...
    for (unsigned long long x = x_min; x <= x_max; x *= 10) { // xを10倍ずつ増やしてテスト
        for (int n = n_min; n <= n_max; n++) {
            // root_floor関数の出力
            __int128_t fast_result = root_ceil((long long)x, n);
            all_tests_passed &= validate_ceil_floor((long long)x, n, fast_result);
        }
    }
    ...
    for (int i = 0; i < random_test_count; ++i) {
        ...
        // root_floor関数の出力
        long long fast_result = root_ceil(x, n);
