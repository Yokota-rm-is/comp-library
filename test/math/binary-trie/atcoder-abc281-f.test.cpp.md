...
data:
  ...
  bundledCode: "#line 1 \"test/math/binary-trie/atcoder-abc281-f.test.cpp\"\n#define\
    ...
    #line 6 \"test/math/binary-trie/atcoder-abc281-f.test.cpp\"\n\nint main() {\n\
    \    ll N;\n    cin >> N;\n\n    vector<ull> A(N);\n    rep(i, N) cin >> A[i];\n\
    \n    BinaryTrie<ull, 64, ll> trie;\n    rep(i, N) {\n        trie.insert(A[i]);\n\
    \    }\n\n    repd(i, 30) {\n        rep(j, trie.A[i].size()) {\n            BinaryTrie<ull,\
    \ 64, ll>::Node* node = trie.A[i][j].get();\n\n            if (node->ch[0] ==\
    \ trie.NIL and node->ch[1] == trie.NIL) continue;\n            else if (node->ch[0]\
    \ == trie.NIL) {\n                node->data = node->ch[1]->data;\n          \
    \      continue;\n            }\n            else if (node->ch[1] == trie.NIL)\
    \ {\n                node->data = node->ch[0]->data;\n                continue;\n\
    \            }\n\n            node->data = min(node->ch[0]->data, node->ch[1]->data)\
  ...
  code: "#define IGNORE\n#define PROBLEM \"https://atcoder.jp/contests/abc281/tasks/abc281_f\"\
    ...
    \ node->ch[1]->data;\n                continue;\n            }\n            else\
    \ if (node->ch[1] == trie.NIL) {\n                node->data = node->ch[0]->data;\n\
    \                continue;\n            }\n\n            node->data = min(node->ch[0]->data,\
    \ node->ch[1]->data) + (1ll << (29 - i));\n        }\n    }\n\n    ll ans = trie.root->data;\n\
    \    cout << ans << endl;\n\n    return 0;\n}"
  dependsOn:
  ...
  verifiedWith: []
documentation_of: test/math/binary-trie/atcoder-abc281-f.test.cpp
layout: document
redirect_from:
- /verify/test/math/binary-trie/atcoder-abc281-f.test.cpp
- /verify/test/math/binary-trie/atcoder-abc281-f.test.cpp.html
