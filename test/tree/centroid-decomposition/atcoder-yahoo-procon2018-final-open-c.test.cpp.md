...
data:
  ...
  bundledCode: "#line 1 \"test/tree/centroid-decomposition/atcoder-yahoo-procon2018-final-open-c.test.cpp\"\
    ...
    \ 6 \"test/tree/centroid-decomposition/atcoder-yahoo-procon2018-final-open-c.test.cpp\"\
    \n\nint main() {\n    ll N, Q;\n    cin >> N >> Q;\n\n    vll A(N - 1), B(N -\
    \ 1);\n    rep(i, N - 1) cin >> A[i] >> B[i];\n    --A, --B;\n\n    vll V(Q),\
    \ K(Q);\n    rep(i, Q) cin >> V[i] >> K[i];\n    --V;\n\n    CentroidDecomposition\
    \ CD(N);\n    rep(i, N - 1) {\n        CD.connect(A[i], B[i]);\n    }\n\n    vvll\
    \ G = CD.G;\n\n    vector<vector<pll>> queries(N);\n    rep(i, Q) {\n        queries[V[i]].emplace_back(K[i],\
    \ i);\n    }\n\n    vll ans(Q, 0);\n    function<void(ll)> dfs = [&](ll root)\
    \ {\n        ll centroid = CD.find_centroid(root);\n\n        vector<array<ll,\
    \ 3>> belongs;\n        vvll depth(G[centroid].size(), vll(1, 0));\n        vll\
    \ depth_all(1, 1);\n\n        rep(i, G[centroid].size()) {\n            if (CD.done[G[centroid][i]])\
    \ continue;\n\n            queue<array<ll, 3>> que;\n            que.push({G[centroid][i],\
    \ centroid, 1});\n            while (!que.empty()) {\n                auto [now,\
    \ p, d] = que.front();\n                que.pop();\n                \n       \
    ...
    \ idx] : queries[v]) {\n                ll x = k - d;\n\n                if (x\
    \ >= 0 and depth_all.size() > x) {\n                    ans[idx] += depth_all[x]\
    \ - (depth[i].size() > x ? depth[i][x] : 0);\n                }\n            }\n\
  ...
  verifiedWith: []
documentation_of: test/tree/centroid-decomposition/atcoder-yahoo-procon2018-final-open-c.test.cpp
layout: document
redirect_from:
- /verify/test/tree/centroid-decomposition/atcoder-yahoo-procon2018-final-open-c.test.cpp
- /verify/test/tree/centroid-decomposition/atcoder-yahoo-procon2018-final-open-c.test.cpp.html
