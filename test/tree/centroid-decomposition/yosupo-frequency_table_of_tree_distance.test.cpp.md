...
data:
  ...
  bundledCode: "#line 1 \"test/tree/centroid-decomposition/yosupo-frequency_table_of_tree_distance.test.cpp\"\
    ...
    \ 4 \"test/tree/centroid-decomposition/yosupo-frequency_table_of_tree_distance.test.cpp\"\
    \n#include <atcoder/all>\nusing atcoder::convolution_ll;\n\nint main() {\n   \
    \ ll N;\n    cin >> N;\n\n    vll A(N - 1), B(N - 1);\n    rep(i, N - 1) cin >>\
    \ A[i] >> B[i];\n\n    CentroidDecomposition CD(N);\n    rep(i, N - 1) {\n   \
    \     CD.connect(A[i], B[i]);\n    }\n\n    vvll G = CD.G;\n\n    vll ans(N -\
    \ 1, 0);\n\n    function<void(ll)> dfs = [&](ll root) {\n        ll centroid =\
    \ CD.find_centroid(root);\n\n        vvll depth(G[centroid].size(), vll(1, 0));\n\
    \        vll depth_all(1, 1);\n\n        rep(i, G[centroid].size()) {\n      \
    \      if (CD.done[G[centroid][i]]) continue;\n\n            queue<array<ll, 3>>\
    \ que;\n            que.push({G[centroid][i], centroid, 1});\n            while\
    \ (!que.empty()) {\n                auto [now, p, d] = que.front();\n        \
    \        que.pop();\n                \n                if (depth[i].size() <=\
    \ d) depth[i].push_back(0);\n                ++depth[i][d];\n                if\
    \ (depth_all.size() <= d) depth_all.push_back(0);\n                ++depth_all[d];\n\
    \n                for (auto next : G[now]) {\n                    if (next ==\
    ...
    \ rep(i, G[centroid].size()) {\n            depth[i] = convolution_ll(depth[i],\
  ...
  verifiedWith: []
documentation_of: test/tree/centroid-decomposition/yosupo-frequency_table_of_tree_distance.test.cpp
layout: document
redirect_from:
- /verify/test/tree/centroid-decomposition/yosupo-frequency_table_of_tree_distance.test.cpp
- /verify/test/tree/centroid-decomposition/yosupo-frequency_table_of_tree_distance.test.cpp.html
