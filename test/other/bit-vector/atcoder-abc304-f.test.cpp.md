...
data:
  ...
  bundledCode: "#line 1 \"test/other/bit-vector/atcoder-abc304-f.test.cpp\"\n#define\
    ...
    \    return result;\n}\n#line 3 \"other/bit-vector.cpp\"\n\ntemplate <long long\
    \ bitsize = 64>\nstruct BitVector {\n    using ull = unsigned long long;\n   \
    \ long long N, L;\n    vector<ull> data;\n\n    BitVector(long long n, bool a\
    ...
    \ r and r <= N);\n\n        auto [pl, ql] = divmod(l);\n        auto [pr, qr]\
    \ = divmod(r - 1);\n\n        if (pl == pr) {\n            ull mask = (~0ull >>\
    \ (bitsize - 1 - (qr - ql))) << ql;\n            data[pl] = a ? (data[pl] | mask)\
    \ : (data[pl] & ~mask);\n        } \n        else {\n            ull maskl = ~0ull\
    \ << ql;\n            data[pl] = a ? (data[pl] | maskl) : (data[pl] & ~maskl);\n\
    \n            rep(p, pl + 1, pr) data[p] = a ? ~0ull : 0ull;\n\n            ull\
    \ maskr = ~0ull >> (bitsize - 1 - qr);\n            data[pr] = a ? (data[pr] |\
    \ maskr) : (data[pr] & ~maskr);\n        }\n    } \n\n    bool get(long long x)\
    \ {\n        assert(0 <= x and x < N);\n\n        auto [p, q] = divmod(x);\n \
    \       return (data[p] >> q) & 1;\n    }\n\n    BitVector get(long long l, long\
    \ long r) {\n        assert(0 <= l and l <= r and r <= N);\n\n        auto [pl,\
    \ ql] = divmod(l);\n\n        BitVector ret(r - l);\n\n        if (l == r) return\
    \ ret;\n\n        if (ql == 0) {\n            rep(i, ret.L) ret.data[i] = data[pl\
    \ + i];\n        } \n        else {\n            rep(i, ret.L) {\n           \
    \     ull low  = data[pl + i] >> ql;\n                ull high = (pl + i + 1 <\
    \ L) ? data[pl + i + 1] << (bitsize - ql) : 0;\n                \n           \
    \     ret.data[i] = low | high;\n            }\n        }\n\n        long long\
    ...
    \ long ret = 0;\n\n        auto [pl, ql] = divmod(l);\n        auto [pr, qr] =\
    \ divmod(r - 1);\n\n        if (pl == pr) {\n            ull mask = (~0ull >>\
    \ (bitsize - 1 - (qr - ql))) << ql;\n            ret += this->popcount(data[pl]\
    \ & mask);\n        } \n        else {\n            ull maskl = ~0ull << ql;\n\
    \            ret += popcount(data[pl] & maskl);\n\n            rep(p, pl + 1,\
    \ pr) ret += popcount(data[p]);\n\n            ull maskr = ~0ull >> (bitsize -\
    ...
    \       n /= p;\n                ++exp;\n            }\n            res.emplace_back(p,\
    \ exp);\n        }\n        return res;\n    }\n\n    // \u9AD8\u901F\u7D04\u6570\
    \u5217\u6319 O(sigma(n))\n    vector<long long> divisors(long long n, bool sorted\
    ...
    \u901F\u30E1\u30D3\u30A6\u30B9\u5909\u63DB\n    // \u5165\u529B F \u304C in-place\
    \ \u306B\u66F4\u65B0\u3055\u308C\u3066\u3001f \u306B\u306A\u308B\n    vector<long\
    \ long> fast_mobius(vector<long long> F) {\n        vector<long long> f(N);\n\
    \        rep(i, N) f[i] = F[i];\n\n        // \u5404\u7D20\u6570 p \u8EF8\u306B\
    \u5BFE\u3057\u3066\n        // \u5C0F\u3055\u3044\u5EA7\u6A19 (k) \u304B\u3089\
    \u5927\u304D\u3044\u5EA7\u6A19 (k * p) \u3092\u5F15\u3044\u3066\u3044\u304F\n\
    \        rep(p, 2, N) {\n            if (!is_prime[p]) continue;\n\n         \
    \   // \u5EA7\u6A19\u304C\u5C0F\u3055\u3044\u65B9\u3092\u8D77\u70B9\u3068\u3057\
    \u3066\u5DEE\u5206\u3092\u3068\u308B\n            for (long long k = 1; k * p\
    \ < N; ++k) {\n                f[k] -= f[k * p];\n            }\n        }\n\n\
    \        return f;\n    }\n\n    // \u6DFB\u5B57 GCD \u7573\u307F\u8FBC\u307F\n\
    ...
    \n        // \u9AD8\u901F\u30E1\u30D3\u30A6\u30B9\u5909\u63DB\n        fast_mobius(H);\n\
    \n        return H;\n    }\n};\n#line 7 \"test/other/bit-vector/atcoder-abc304-f.test.cpp\"\
    \n\nusing mint = mint998;\n\nint main() {\n    ll N;\n    cin >> N;\n\n    string\
    \ S;\n    cin >> S;\n\n    BitVector bv(2 * N, 1);\n    rep(i, N) if (S[i] ==\
  ...
  requiredBy: []
  timestamp: '2025-11-16 21:46:48+09:00'
  verificationStatus: TEST_WRONG_ANSWER
  verifiedWith: []
documentation_of: test/other/bit-vector/atcoder-abc304-f.test.cpp
layout: document
redirect_from:
- /verify/test/other/bit-vector/atcoder-abc304-f.test.cpp
- /verify/test/other/bit-vector/atcoder-abc304-f.test.cpp.html
