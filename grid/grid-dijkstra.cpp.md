...
data:
  ...
  bundledCode: "#line 2 \"base.cpp\"\n\n#include <bits/stdc++.h>\n// #include <atcoder/all>\n\
    ...
    \ start) {\n        dijkstra(vector<Coordinate>{start});\n    }\n\n    void dijkstra(vector<Coordinate>\
    \ starts) {\n        fore(start, starts) {\n            assert(!done(start));\n\
    \            assert(!field.is_out(start));\n            assert(!field.is_obj(start));\n\
    \        }\n\n        priority_queue<pair<long long, Coordinate>, vector<pair<long\
    \ long, Coordinate>>, greater<>> que;\n\n        fore(start, starts) {\n     \
    \       que.emplace(0, start);\n        }\n\n        while (!que.empty()) {\n\
    \            auto p = que.top();\n            que.pop();\n\n            Coordinate\
    \ now = p.second;\n            done(now) = true;\n            cost(now) = p.first;\n\
    \            cc(now) = group;\n\n            rep(i, dirs.size()) {\n         \
...
documentation_of: grid/grid-dijkstra.cpp
layout: document
redirect_from:
- /library/grid/grid-dijkstra.cpp
- /library/grid/grid-dijkstra.cpp.html
