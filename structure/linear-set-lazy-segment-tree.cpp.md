...
data:
  ...
  bundledCode: "#line 2 \"base.cpp\"\n\n#include <bits/stdc++.h>\n// #include <atcoder/all>\n\
    ...
    \ = pow(x, 1.0 / n); if (binpow(ret, n) < x) ++ret; return ret;}\n#line 3 \"structure/linear-set-lazy-segment-tree.cpp\"\
    \n\ntemplate<typename T>\nstruct Node {\n    T sum, min, max;\n    long long index_min,\
    \ index_max;\n    long long left, right;\n\n    Node(T s, T m, T M, long long\
    \ i_min = -1, long long i_max = -1, long long l = -1, long long r = -1) : sum(s),\
    \ min(m), max(M), index_min(i_min), index_max(i_max), left(l), right(r) {\n  \
    \  };\n\n    Node(array<T, 3> a, long long i_min = -1, long long i_max = -1, long\
    \ long l = -1, long long r = -1) : sum(a[0]), min(a[1]), max(a[2]), index_min(i_min),\
    \ index_max(i_max), left(l), right(r) {\n    };\n\n    bool operator== (array<T,\
    ...
    \ {};\n\n    array<T, 3> e() {\n        return _e;\n    }\n\n    S operator()\
    \ (const S& x, const S& y) {\n        T s = x.sum + y.sum;\n        T m = min(x.min,\
    \ y.min);\n        T M = max(x.max, y.max);\n        long long index_min = (y.min\
    \ < x.min ? y.index_min : x.index_min);\n        long long index_max = (y.max\
    \ > x.max ? y.index_max : x.index_max);\n        long long left = (x.left != -1\
    ...
    \ F f) {\n        if (f == id()) return;\n\n        x.sum = (f.first * (x.left\
    \ + x.right - 1) + f.second * 2) * (x.right - x.left) / 2;\n        x.min = f.first\
    ...
    \n        rep(i, N) {\n            if (i < _N) node[i + N] = S(v[i], v[i], v[i],\
  ...
  code: "#pragma once\n#include \"../base.cpp\"\n\ntemplate<typename T>\nstruct Node\
    ...
    \ left, right;\n\n    Node(T s, T m, T M, long long i_min = -1, long long i_max\
    \ = -1, long long l = -1, long long r = -1) : sum(s), min(m), max(M), index_min(i_min),\
    \ index_max(i_max), left(l), right(r) {\n    };\n\n    Node(array<T, 3> a, long\
    \ long i_min = -1, long long i_max = -1, long long l = -1, long long r = -1) :\
    \ sum(a[0]), min(a[1]), max(a[2]), index_min(i_min), index_max(i_max), left(l),\
    \ right(r) {\n    };\n\n    bool operator== (array<T, 3> a) {\n        return\
    \ (sum == a[0] and min == a[1] and max == a[2]);\n    }\n\n    friend ostream&\
    \ operator << (ostream &os, const Node<T>& node) {\n        return os << \"(\"\
    \ << node.sum << \", \" << node.min << \", \" << node.max << \")\";\n    }\n};\n\
    \ntemplate<typename T>\nstruct Operation {\n    using S = Node<T>;\n\n    Operation():\
    \ _e({T(0), numeric_limits<T>::max(), numeric_limits<T>::min()}) {};\n\n    array<T,\
    \ 3> e() {\n        return _e;\n    }\n\n    S operator() (const S& x, const S&\
    \ y) {\n        T s = x.sum + y.sum;\n        T m = min(x.min, y.min);\n     \
    \   T M = max(x.max, y.max);\n        long long index_min = (y.min < x.min ? y.index_min\
    \ : x.index_min);\n        long long index_max = (y.max > x.max ? y.index_max\
    \ : x.index_max);\n        long long left = (x.left != -1 ? x.left : y.left);\n\
    \        long long right = (y.right != -1 ? y.right : x.right);\n\n        S ret(s,\
    \ m, M, index_min, index_max, left, right);\n\n        return ret;\n    }\n\n\
    private:\n    array<T, 3> _e;\n};\n\ntemplate<typename T, typename F2>\nstruct\
    \ Mapping {\n    using S = Node<T>;\n    using F = pair<F2, F2>;\n\n    Mapping():\
    \ _id(numeric_limits<F2>::max(), numeric_limits<F2>::max()) {};\n\n    F id()\
    \ {\n        return _id;\n    }\n\n    void operator() (S &x, const F f) {\n \
    \       if (f == id()) return;\n\n        x.sum = (f.first * (x.left + x.right\
    \ - 1) + f.second * 2) * (x.right - x.left) / 2;\n        x.min = f.first * x.left\
    \ + f.second;\n        x.max = f.first * (x.right - 1) + f.second;\n        x.index_min\
    \ = x.left;\n        x.index_max = x.right - 1;\n\n        if (f.first < 0) {\n\
  ...
  requiredBy: []
  timestamp: '2024-09-28 16:58:11+09:00'
  verificationStatus: LIBRARY_ALL_AC
  verifiedWith:
  - test/structure/linear-set-lazy-segment-tree/atcoder-abc177-f.test.cpp
documentation_of: structure/linear-set-lazy-segment-tree.cpp
layout: document
redirect_from:
- /library/structure/linear-set-lazy-segment-tree.cpp
- /library/structure/linear-set-lazy-segment-tree.cpp.html
