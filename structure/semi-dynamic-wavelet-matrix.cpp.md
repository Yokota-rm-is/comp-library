...
data:
  ...
  code: "#pragma once\n#include \"../base.cpp\"\n\ntemplate <typename T>\nstruct BIT\
    ...
    \u6570\n\n    vector<BIT<ull>> state; // active\u3067\u306A\u3044\u306A\u3089\
    1\n\n    SemiDynamicBitVector(ull n) : N(n), state(2, n) {\n        const ull\
    \ s = (n + blockBitNum - 1) / blockBitNum + 1;   // ceil(n, blockSize)\n     \
    ...
    \       }\n        else return pos - rank(1, pos);\n    }\n\n    ull active_rank(const\
    \ ull bit, const ull pos) {\n        assert(bit == 0 or bit == 1);\n        assert(pos\
    \ <= N);\n\n        return rank(bit, pos) - state[bit].sum(pos);\n    }\n\n  \
    \  // k\u756A\u76EE(1-indexed)\u306Ebit\u306E\u4F4D\u7F6E(0-indexed)\u3092\u8FD4\
    \u3059\n    // \u5B58\u5728\u3057\u306A\u3044\u5834\u5408\u306FN\u3092\u8FD4\u3059\
    \n    ull select(const ull bit, const ull k) {\n        assert(bit == 0 or bit\
...
layout: document
redirect_from:
- /library/structure/semi-dynamic-wavelet-matrix.cpp
- /library/structure/semi-dynamic-wavelet-matrix.cpp.html
