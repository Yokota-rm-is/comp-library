...
data:
  ...
  bundledCode: "#line 2 \"base.cpp\"\n\n#include <bits/stdc++.h>\n// #include <atcoder/all>\n\
    ...
    \ x) { return __builtin_popcountll(x); }\n#endif\n#line 2 \"geometry/polyomino.cpp\"\
    \n\nstruct Polyomino {\n    vector<vector<long long>> mino;\n    ll left, right,\
    \ top, bottom;\n\n    Polyomino(const vector<vector<long long>> &m, ll l, ll t)\
    \ : mino(m), left(l), right(l + m.front().size()), top(t), bottom(t + m.size())\
    \ {}\n\n    // v[i] = {x, y}; \n    Polyomino(const vector<pair<ll, ll>> &v, ll\
    \ l, ll t) :left(l), top(t) {\n        ll h = 0, w = 0;\n        rep(i, v.size())\
    \ {\n            chmax(w, v[i].first);\n            chmax(h, v[i].second);\n \
    \       }\n\n        mino.assign(h, vector<long long>(w, 0));\n        rep(i,\
    \ v.size()) {\n            mino[v[i].second][v[i].first] = 1;\n        }\n\n \
    \       right = left + w;\n        bottom = top + h;\n    }\n\n    bool is_overlapped(Polyomino\
    \ &other) {\n        return !is_not_overlapped(other);\n    }\n\n    bool is_not_overlapped(Polyomino\
    \ &other) {\n        if (right <= other.left or other.right <= left or bottom\
    \ <= other.top or other.bottom <= top) return true;\n        \n        rep(y,\
    \ max(top, other.top), min(bottom, other.bottom)) rep(x, max(left, other.left),\
    \ min(right, other.right)) {\n            if (check(x, y) == 1 and other.check(x,\
    \ y) == 1) return false;\n        }\n\n        return true;\n    }\n\n    long\
    \ long check(long long x, long long y) {\n        if (x < left or x >= right)\
    \ return 0;\n        if (y < top or y >= bottom) return 0;\n\n        return mino[y\
    \ - top][x - left];\n    }\n\n    friend constexpr ostream& operator << (ostream\
    \ &os, const Polyomino& p) noexcept {\n        rep(y, p.mino.size()) {\n     \
    \       rep(x, p.mino.front().size()) {\n                os << p.mino[y][x] <<\
    \ \" \";\n            }\n            os << endl;\n        }\n        return os;\n\
    \    }\n};\n"
  code: "#include \"../base.cpp\"\n\nstruct Polyomino {\n    vector<vector<long long>>\
    \ mino;\n    ll left, right, top, bottom;\n\n    Polyomino(const vector<vector<long\
    \ long>> &m, ll l, ll t) : mino(m), left(l), right(l + m.front().size()), top(t),\
    \ bottom(t + m.size()) {}\n\n    // v[i] = {x, y}; \n    Polyomino(const vector<pair<ll,\
    \ ll>> &v, ll l, ll t) :left(l), top(t) {\n        ll h = 0, w = 0;\n        rep(i,\
    \ v.size()) {\n            chmax(w, v[i].first);\n            chmax(h, v[i].second);\n\
    \        }\n\n        mino.assign(h, vector<long long>(w, 0));\n        rep(i,\
    \ v.size()) {\n            mino[v[i].second][v[i].first] = 1;\n        }\n\n \
    \       right = left + w;\n        bottom = top + h;\n    }\n\n    bool is_overlapped(Polyomino\
  ...
  verifiedWith: []
documentation_of: geometry/polyomino.cpp
layout: document
redirect_from:
- /library/geometry/polyomino.cpp
- /library/geometry/polyomino.cpp.html
