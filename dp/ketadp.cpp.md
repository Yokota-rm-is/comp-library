...
data:
  ...
  bundledCode: "#line 2 \"base.cpp\"\n\n#include <bits/stdc++.h>\n// #include <atcoder/all>\n\
    ...
    \ x) { return __builtin_popcountll(x); }\n#endif\n#line 2 \"dp/ketadp.cpp\"\n\n\
    template<typename T, auto F>\nstruct DP {\n    string N;\n    long long L, K;\n\
    \    vector<vector<vector<T>>> dp;\n\n    DP (string n, long long k): N(n), L(n.size()),\
    \ K(k) {\n        dp.resize(L + 1, vector(2, vector<ll>(K, -1)));\n    };\n\n\
    \    long long multiple_of_K() {\n        dp[0][false][0] = 0;\n\n        rep(i,\
    \ L) {\n            rep(j, K) {\n                rep(d, 10) {\n              \
    \      if (dp[i][true][j] < 0) continue;\n\n                    dp[i + 1][true][F(j,\
    \ d)] += dp[i][true][j];\n                }\n\n                ll a = N[i] - '0';\n\
    \n                rep(d, a) {\n                    if (dp[i][false][j] < 0) continue;\
    \          \n\n                    dp[i + 1][true][F(j, d)] += dp[i][false][j];\
    \ \n                }\n\n                dp[i + 1][false][F(j, a)] += dp[i][false][j];\n\
    \            }\n        }\n\n        return dp[L][true][0] + dp[L][false][0];\n\
    \    }\n};\n\n// \u5404\u4F4D\u306E\u548C\u306ED\u306Emod\ntemplate<long long\
    \ D>\nlong long digit_sum_mod(long long j, long long d) {\n    return (j + d)\
    \ % D;\n}\n\n// \ntemplate<auto S>\nlong long contains(long long j, long long\
    \ d) {\n    return j || S.contains(d);\n}\n"
  code: "#include \"../base.cpp\"\n\ntemplate<typename T, auto F>\nstruct DP {\n \
    \   string N;\n    long long L, K;\n    vector<vector<vector<T>>> dp;\n\n    DP\
    \ (string n, long long k): N(n), L(n.size()), K(k) {\n        dp.resize(L + 1,\
    \ vector(2, vector<ll>(K, -1)));\n    };\n\n    long long multiple_of_K() {\n\
    \        dp[0][false][0] = 0;\n\n        rep(i, L) {\n            rep(j, K) {\n\
    \                rep(d, 10) {\n                    if (dp[i][true][j] < 0) continue;\n\
    \n                    dp[i + 1][true][F(j, d)] += dp[i][true][j];\n          \
    \      }\n\n                ll a = N[i] - '0';\n\n                rep(d, a) {\n\
    \                    if (dp[i][false][j] < 0) continue;          \n\n        \
    \            dp[i + 1][true][F(j, d)] += dp[i][false][j]; \n                }\n\
    \n                dp[i + 1][false][F(j, a)] += dp[i][false][j];\n            }\n\
    ...
    \ digit_sum_mod(long long j, long long d) {\n    return (j + d) % D;\n}\n\n//\
    \ \ntemplate<auto S>\nlong long contains(long long j, long long d) {\n    return\
    \ j || S.contains(d);\n}"
  dependsOn:
  - base.cpp
  ...
  verifiedWith: []
documentation_of: dp/ketadp.cpp
layout: document
redirect_from:
- /library/dp/ketadp.cpp
- /library/dp/ketadp.cpp.html
title: dp/ketadp.cpp
---
