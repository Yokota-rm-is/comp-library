...
data:
  ...
  bundledCode: "#line 2 \"base.cpp\"\n\n#include <bits/stdc++.h>\n// #include <atcoder/all>\n\
    ...
    \n    pN pNIL;\n    Node *NIL = nullptr;\n    vector<pN> A;\n    Node *root;\n\
    \n    T a;\n    long long node_id, word_id;\n\n    Trie(T a = T()) : a(a), node_id(0),\
    \ word_id(0) {\n        pNIL = make_unique<Node>();\n        NIL = pNIL.get();\n\
    \        NIL->p = NIL;\n        NIL->id = node_id++;\n        NIL->c = 0;\n  \
    \      NIL->common = 0;\n        NIL->depth = 0;\n        NIL->data = a;\n   \
    \     NIL->accepts = set<long long>();\n        NIL->ch = unordered_map<char,\
    \ Node*>();\n        root = NIL;\n    }\n\n    Node* new_node(char c, long long\
    \ d) {\n        pN pnx = make_unique<Node>();\n        Node *nx = pnx.get();\n\
    \        nx->p = NIL;\n        nx->id = node_id++;\n        nx->c = c;\n     \
    \   nx->common = 0;\n        nx->depth = d;\n        nx->data = a;\n        nx->accepts\
    \ = set<long long>();\n        nx->ch = unordered_map<char, Node*>();\n\n    \
    \    A.emplace_back(move(pnx));\n        return nx;\n    }\n\n    // \u5358\u8A9E\
    \u306E\u633F\u5165\n    Node* insert(const string& word, long long n = 1, function<void(Node*,\
    \ const string&)> f = [](Node* n, const string& word){}) {\n        Node *node\
    \ = root;\n        node->common += n;\n        f(node, word);\n\n        rep(i,\
    \ word.size()){\n            char c = word[i];\n            node = push_back(node,\
    \ c, n);\n\n            if (i == word.size() - 1) {\n                rep(j, n)\
    \ node->accepts.insert(word_id++);\n            }\n\n            f(node, word);\n\
    \        }\n\n        return node;\n    }\n\n    // \u5358\u8A9E\u306E\u633F\u5165\
    \n    Node* insert(const string& word, function<void(Node*, const string&)> f)\
    \ {\n        return insert(word, 1, f);\n    }\n\n    Node* push_back(Node* node,\
    \ char c, long long n = 1) {\n        Node *next_node;\n        if (node->ch.contains(c))\
    \ next_node = node->ch[c];\n        else {\n            next_node = new_node(c,\
    \ node->depth + 1);\n            next_node->p = node;\n            node->ch[c]\
    \ = next_node;\n        }\n\n        next_node->common += n;\n\n        return\
    \ next_node;\n    }\n\n    vector<long long> erase(const string& word, long long\
    \ n = 1, function<void(Node*, const string&)> f = [](Node* n, const string& word){})\
    ...
    \n            if (node == root) break;\n            node = node->p;\n        }\n\
    \n        return ret;\n    }\n\n    vector<long long> erase(const string& word,\
    \ function<void(Node*, const string&)> f) {\n        return erase(word, 1, f);\n\
    \    }\n\n    void erase_with_prefix(const string& prefix, function<void(Node*,\
    \ const string&)> f = [](Node* n, const string& prefix){}) {\n        Node* node\
    ...
    \            Node* next_node;\n            if (node->ch.contains(c)) next_node\
    \ = node->ch[c];\n            else break;\n            \n            node = next_node;\n\
    \        }\n\n        return node;\n    }\n\n    bool contains(const string& word)\
    \ {\n        Node* node = search(word);\n        return node->depth == word.size()\
    \ and !node->accepts.empty();\n    }\n\n    // prefix \u3092\u6301\u3064\u5358\
    \u8A9E\u304C\u5B58\u5728\u3059\u308B\u304B\u306E\u691C\u7D22\n    bool contains_with_prefix(const\
    \ string& prefix) {\n        Node* node = search(prefix);\n        return node->depth\
    \ == prefix.size();\n    }\n\n    long long count(const string& word) {\n    \
    \    Node* node = search(word);\n        return (node->depth == word.size() ?\
    \ node->accepts.size() : 0);\n    }\n\n    long long count_with_prefix(const string&\
    \ prefix) {\n        Node* node= search(prefix);\n        return (node->depth\
    \ == prefix.size() ? node->common : 0);\n    }\n\n    // \u633F\u5165\u3057\u305F\
  ...
  code: "#pragma once\n#include \"../base.cpp\"\n\ntemplate <typename T = long long>\n\
    ...
    \            f(node, word);\n\n            if (node == root) break;\n        \
    \    node = node->p;\n        }\n\n        return ret;\n    }\n\n    vector<long\
    \ long> erase(const string& word, function<void(Node*, const string&)> f) {\n\
    \        return erase(word, 1, f);\n    }\n\n    void erase_with_prefix(const\
    \ string& prefix, function<void(Node*, const string&)> f = [](Node* n, const string&\
    \ prefix){}) {\n        Node* node = search(prefix);\n\n        if (node->depth\
    \ != prefix.size()) return;\n        \n        long long n = node->common; \n\n\
    \        repd(i, prefix.size() + 1) {\n            pop_back(node, n);\n      \
    \      f(node, prefix);\n\n            if (node == root) break;\n            node\
    \ = node->p;\n        }\n    }\n\n    void pop_back(Node* node, long long n =\
    \ 1) {\n        node->common -= n;\n\n        if (node->common == 0 and node !=\
    ...
    \u6700\u5927\u4E00\u81F4\u3059\u308B\u30CE\u30FC\u30C9\u3092\u8FD4\u3059\n   \
    \ // f: \u63A2\u7D22\u3057\u306A\u304C\u3089\u51E6\u7406\u3057\u305F\u3044\u95A2\
    \u6570\n    Node* search(const string& word, function<void(Node*)> f = [](Node*\
    \ n){}) {\n        Node* node = root;\n\n        rep(i, word.size() + 1) {\n \
    \           f(node);\n            if (i == word.size()) break;\n\n           \
    \ char c = word[i];\n            Node* next_node;\n            if (node->ch.contains(c))\
    \ next_node = node->ch[c];\n            else break;\n            \n          \
    \  node = next_node;\n        }\n\n        return node;\n    }\n\n    bool contains(const\
    \ string& word) {\n        Node* node = search(word);\n        return node->depth\
    \ == word.size() and !node->accepts.empty();\n    }\n\n    // prefix \u3092\u6301\
    \u3064\u5358\u8A9E\u304C\u5B58\u5728\u3059\u308B\u304B\u306E\u691C\u7D22\n   \
    \ bool contains_with_prefix(const string& prefix) {\n        Node* node = search(prefix);\n\
    \        return node->depth == prefix.size();\n    }\n\n    long long count(const\
    \ string& word) {\n        Node* node = search(word);\n        return (node->depth\
    \ == word.size() ? node->accepts.size() : 0);\n    }\n\n    long long count_with_prefix(const\
    \ string& prefix) {\n        Node* node= search(prefix);\n        return (node->depth\
    \ == prefix.size() ? node->common : 0);\n    }\n\n    // \u633F\u5165\u3057\u305F\
    \u5358\u8A9E\u306E\u6570\n    long long count() const {\n        return root->common;\n\
    \    }\n\n    // Trie\u6728\u306E\u30CE\u30FC\u30C9\u6570\n    long long size()\
  ...
  requiredBy: []
  timestamp: '2025-04-30 21:39:50+09:00'
