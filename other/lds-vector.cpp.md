...
data:
  ...
  code: "#pragma once\n#include \"../base.cpp\"\n\ntemplate <typename T = long long,\
    ...
    \ deque<deque<T>> table;\n\n    LDSVector() {}\n    LDSVector(const vector<T>&\
    \ v) {\n        for (auto& x : v) push_back(x);\n    }\n\n    void push_back(T\
    \ val) {\n        vals.push_back(val);\n\n        if (dp.empty() or dp.back()\
    \ > val or (not strict and dp.back() == val)) {\n            prev.push_back(dp.empty()\
    \ ? -1 : dp.back());\n            dp.push_back(val);\n            table.push_back({val});\n\
    \        } \n        else {            \n            long long idx = get_index(val);\n\
    \n            dp[idx] = val;\n            table[idx].push_back(val);\n       \
    \     prev.push_back(idx == 0 ? -1 : dp[idx - 1]);\n        }\n    }\n\n    void\
...
documentation_of: other/lds-vector.cpp
layout: document
redirect_from:
- /library/other/lds-vector.cpp
- /library/other/lds-vector.cpp.html
