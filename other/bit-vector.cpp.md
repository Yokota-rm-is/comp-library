...
data:
  ...
  code: "#pragma once\n#include \"../base.cpp\"\n\ntemplate <long long bitsize = 64>\n\
    ...
    \        \n        auto [p, q] = divmod(x);\n        if (a) data[p] |= (1ll <<\
    \ q);\n        else data[p] &= ~(1ll << q);\n    };\n\n    void set(long long\
    \ l, long long r, bool a) {\n        assert(0 <= l and l <= r and r <= N);\n\n\
    \        auto [pl, ql] = divmod(l);\n        auto [pr, qr] = divmod(r - 1);\n\n\
    \        if (pl == pr) {\n            ull mask = (~0ull >> (bitsize - 1 - (qr\
    \ - ql))) << ql;\n            data[pl] = a ? (data[pl] | mask) : (data[pl] & ~mask);\n\
    \        } \n        else {\n            ull maskl = ~0ull << ql;\n          \
    \  data[pl] = a ? (data[pl] | maskl) : (data[pl] & ~maskl);\n\n            rep(p,\
    \ pl + 1, pr) data[p] = a ? ~0ull : 0ull;\n\n            ull maskr = ~0ull >>\
    \ (bitsize - 1 - qr);\n            data[pr] = a ? (data[pr] | maskr) : (data[pr]\
    \ & ~maskr);\n        }\n    } \n\n    bool get(long long x) {\n        assert(0\
    \ <= x and x < N);\n\n        auto [p, q] = divmod(x);\n        return (data[p]\
...
documentation_of: other/bit-vector.cpp
layout: document
redirect_from:
- /library/other/bit-vector.cpp
- /library/other/bit-vector.cpp.html
