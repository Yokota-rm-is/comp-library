...
data:
  ...
  code: "#pragma once\n#include \"../base.cpp\"\n\ntemplate <typename T>\nstruct CumulativeSum2D\
    ...
    \ = false;\n\n    CumulativeSum2D(long long h, long long w) : H(h), W(w), values(H,\
    \ vector<T>(W, 0)), sums(H + 1, vector<T>(W + 1, 0)) {}\n    CumulativeSum2D(long\
    \ long h, long long w, T a) : H(h), W(w), values(H, vector<T>(W, a)), sums(H +\
    \ 1, vector<T>(W + 1, 0)) {}\n\n    CumulativeSum2D(const vector<vector<T>>& A)\
    \ : H(A.size()), W(A.front().size()), values(A), sums(H + 1, vector<T>(W + 1,\
    \ 0)) {\n        build(A);\n    };\n\n    void add(long long y, long long x, T\
    \ a) {\n        assert(0 <= y and y < H);\n        assert(0 <= x and x < W);\n\
    \        assert(not built);\n        values[y][x] += a;\n    }\n\n    void build()\
    \ {\n        assert(not built);\n        built = true;\n        rep(i, H) rep(j,\
...
layout: document
redirect_from:
- /library/other/cumulative-sum-2d.cpp
- /library/other/cumulative-sum-2d.cpp.html
